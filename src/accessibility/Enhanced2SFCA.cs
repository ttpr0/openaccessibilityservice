...
namespace DVAN.Accessibility
{
    ...
    {
        public static async Task<float[]> calc2SFCA(IPopulationView population, double[][] facilities, double[] capacities, List<double> ranges, IDistanceDecay decay, IRoutingProvider provider, string? mode)
        {
            ...
            var matrix = await provider.requestTDMatrix(population, facilities, ranges, mode);
            if (matrix == null) {
